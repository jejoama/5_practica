...
dirty_iris %>% arrange(desc(dirty_iris$Species)) 

#de menor a mayor --< sin funcion porque por defecto lo hace de menor a mayor
...
dirty_iris %>% arrange (dirty_iris$Species)
...
dirty_iris_clean <- dirty_iris %>% mutate(across(where(is.numeric), ~ ifelse(!is.finite(.) | . <= 0, NA, .)))
...
dff <- sum(complete.cases(dirty_iris_clean)) # el numero de observaciones
dfff <- sum(complete.cases(dirty_iris_clean)*100/nrow(dirty_iris_clean)) #porcentaje

clean_iris <- na.omit(dirty_iris_clean)
```
...
clean_clean <- clean_iris %>%
  summarise(across(where(is.numeric), list(
    media = ~mean(.x, na.rm = TRUE),
    desv = ~sd(.x, na.rm = TRUE),
    media_abs = ~mad(.x, na.rm = TRUE),
    ran_in = ~IQR(.x, na.rm = TRUE)
  )))
...
```{r , include = FALSE}
# Regla 3 sigma
tres_sigma <- function(x){
  media = mean(x, na.rm=T)
  desv = sd(x, na.rm = T)
  si_no = (x <= media-3*desv | x >= media+3*desv)
  ifelse(is.na(x), NA, si_no)
}
...
hhampel <- function(x){
  ...
  no_si = (x <= mediana-3*MADM | x >= mediana-3*MADM)
  ifelse(is.na(x), NA, no_si)
}


# Regla boxplot
boxx <- function(x){
  v_cinco = quantile(x, probs=0.25)
  v_siete = quantile(x, probs=0.75)
  disp = IQR(v_cinco - v_siete)
  no__si= (x<v_cinco-1.5*disp | x>v_siete+1.5*disp)
  ifelse(is.na(x), NA, no__si)
}

# Percentiles
percetil <- function(x){
  v_s_cinco =  quantile(x, probs=0.05, na.rm=T) 
  v_n_cinco = quantile(x, probs=0.95, na.rm=T) 
  condi = (x<v_s_cinco | x> v_n_cinco)
  ifelse(is.na(x), NA, condi)
}
...
clean_iris_outliers <- clean_iris %>%
  mutate(
    out_sigma = tres_sigma(Petal.Length),
    out_hampel = hhampel(Petal.Length),
    out_boxplot = boxx(Petal.Length),
    out_percentil = percetil(Petal.Length)
  )
...
clean_iris_outliers %>%
  summarise(
    sigma_total = sum(out_sigma, na.rm = TRUE),
    hampel_total = sum(out_hampel, na.rm = TRUE),
    box_total = sum(out_boxplot, na.rm = TRUE),
    perc_total = sum(out_percentil, na.rm = TRUE)
  )
...
# reestructurar con pivot_longer
iris_long <- clean_iris_outliers %>%
  select(Petal.Length, out_sigma, out_hampel, out_boxplot, out_percentil) %>%
  pivot_longer(
...
iris_long <- iris_long %>%
  mutate(Metodo = case_when(
    Metodo == "out_sigma" ~ "3 Sigma",
    Metodo == "out_hampel" ~ "Hampel",
    Metodo == "out_boxplot" ~ "Boxplot",
    Metodo == "out_percentil" ~ "Percentil"
  ))


ggplot(iris_long, aes(x = Metodo, y = Petal.Length, color = Outlier)) +
